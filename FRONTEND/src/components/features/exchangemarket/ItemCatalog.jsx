    ...
    <div className="flex flex-col">
            ...
            <>
              <div className="bg-white p-4 rounded-lg border border-gray-200 flex flex-col gap-4">
                ...
                <div className="flex flex-wrap gap-2">
                  {(marketView === "my_items" ? userItemStatuses : marketplaceCategories).map((filterItem) => {
                    const isActive = marketView === "my_items" ? marketStatusFilter === filterItem.key : marketCategory === filterItem.key;
                    const filterKey = filterItem.key;
                    const filterName = filterItem.name;
                    const Icon = marketView === "my_items" && filterItem.icon;
                    const statusColor = marketView === "my_items" ? statusColors[filterKey] || statusColors.all : isActive ? "bg-emerald-100 text-emerald-700 font-medium" : "bg-gray-100 text-gray-600 hover:bg-gray-200";

                    return (
                      <button
                        key={filterKey}
                        className={`px-3 py-1.5 rounded-lg text-sm flex items-center transition-all duration-150 ${isActive ? (marketView === "my_items" ? `${statusColor} font-medium shadow-sm` : "bg-emerald-100 text-emerald-700 font-medium") : "bg-gray-100 text-gray-600 hover:bg-gray-200"}`}
                        onClick={() => marketView === "my_items" ? setMarketStatusFilter(filterKey) : setMarketCategory(filterKey)}
                      >
                        ...
                        {isActive && marketView === "my_items" && (
                          <span className="ml-1.5 bg-white bg-opacity-30 text-xs font-normal rounded-full px-1.5 py-0.5">
                            {filteredMarketItems.filter((item) => filterKey === "all" ? true : item.status === filterKey).length}
                          </span>
                        )}
                      </button>
                    );
              ...
              {filteredMarketItems.length === 0 ? (
                <div className="text-center py-10 bg-white rounded-lg border border-gray-200">
                  ...
                  <h3 className="text-lg font-medium text-gray-700 mb-2">Không có sản phẩm nào phù hợp</h3>
                  <p className="text-gray-500 max-w-md mx-auto mb-6">
                    {marketView === "my_items" ? "Bạn chưa có sản phẩm nào với trạng thái này." : "Hiện chưa có sản phẩm nào trong danh mục này."}
                    {marketView === "my_items" && marketStatusFilter === "all" && "Hãy thử thêm sản phẩm mới!"}
                  </p>
                  {marketView === "my_items" && (
                    <button
                ...
                <div className={marketListView === "grid" ? "grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4" : "space-y-4"}>
                  {marketListView === "list" ? (
                      ...
                      <table className="min-w-full divide-y divide-gray-200">
                        <thead className="bg-gray-50">
                          <tr>
                            ...
                            {marketView === "my_items" && <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Trạng thái</th>}
                        ...
                        <tbody className="bg-white divide-y divide-gray-200">
                          {filteredMarketItems.map((item) => (
                            <tr key={item.id} className="hover:bg-gray-50">
                              ...
                              <td className="px-6 py-4 whitespace-nowrap">
                                <div className="text-sm text-gray-500">{getCategoryDisplayName(item.category)}</div>
                              </td>
                              <td className="px-6 py-4 whitespace-nowrap">
                                <div className="flex items-center text-sm font-medium text-amber-600">
                                  {item.price}
                                  <img src="/assets/icons/coin.png" alt="coins" className="ml-1 h-4 w-4" />
                                </div>
                              </td>
                              {marketView === "my_items" && (
                                <td className="px-6 py-4 whitespace-nowrap">
                                  <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${statusColors[item.status] || statusColors.draft}`}>
                              ...
                              <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div className="flex justify-end gap-2">
                                  {marketView === "my_items" ? (
                                    <>
                                      <button onClick={() => handleEditItem(item)} className="text-indigo-600 hover:text-indigo-900">Sửa</button>
                                      <button onClick={() => handleDeleteItem(item.id)} className="text-red-600 hover:text-red-900">Xóa</button>
                                    </>
                                  ) : (
                                    <>
                                      <button onClick={() => { setSelectedItem(item); setShowDetailsModal(true); }} className="text-emerald-600 hover:text-emerald-800">Xem chi tiết</button>
                                      <button onClick={() => { setSelectedItem(item); handlePurchase(item); }} className="text-indigo-600 hover:text-indigo-900" disabled={!item.canPurchase}>Mua</button>
                                    </>
                                  )}
                                </div>
                              </td>
      ...
      {selectedItem && (
        <>
          ...
          />
          <div className={`fixed inset-0 z-50 flex items-center justify-center ${showDetailsModal ? "visible" : "invisible"}`}>
            <div className="absolute inset-0 bg-black bg-opacity-30" onClick={() => setShowDetailsModal(false)}></div>
            <div className="relative bg-white rounded-lg shadow-xl max-w-lg w-full p-6 z-10 mx-4">
              <button onClick={() => setShowDetailsModal(false)} className="absolute top-4 right-4 text-gray-400 hover:text-gray-600"><span className="text-xl">✕</span></button>
              <h2 className="text-2xl font-semibold mb-4">{selectedItem.name}</h2>
              <div className="mb-4">
                <img src={selectedItem.image || "/placeholder.svg"} alt={selectedItem.name} className="w-full h-48 object-cover rounded-lg" />
              </div>
              <div className="mb-3 flex justify-between items-center">
                <span className="text-lg font-semibold text-amber-600 flex items-center">
                  {selectedItem.price}
                  <img src="/assets/icons/coin.png" alt="coins" className="ml-1 h-5 w-5" />
                </span>
                <span className="bg-gray-100 text-gray-700 px-3 py-1 rounded-full text-sm">{getCategoryDisplayName(selectedItem.category)}</span>
              </div>
              <div className="mb-4">
              ...
              <div className="mb-4">
                ...
                <div className="grid grid-cols-2 gap-2 text-sm">
                  ...
                  <div className="text-gray-700 font-medium">{selectedItem.stock || "Không xác định"}</div>
                  <div className="text-gray-500">Tình trạng:</div>
                  <div className="text-gray-700 font-medium">{selectedItem.condition === "new" ? "Mới" : "Cũ"}</div>
                  <div className="text-gray-500">Ngày đăng:</div>
                  <div className="text-gray-700 font-medium">{selectedItem.createdAt && new Date(selectedItem.createdAt).toLocaleDateString()}</div>
                </div>
              </div>
              <div className="flex justify-end space-x-3 mt-6">
                <button onClick={() => setShowDetailsModal(false)} className="px-4 py-2 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50">Đóng</button>
                <button
                  onClick={() => { setShowDetailsModal(false); setIsModalOpen(true); }}
